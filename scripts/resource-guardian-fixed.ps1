...
$hourlyLimits = @{}
$hourlyLimits.Add('dashscope-coding-plan/qwen3.5-plus', 100)
$hourlyLimits.Add('dashscope-coding-plan/qwen3-coder-plus', 100)
$hourlyLimits.Add('dashscope-coding-plan/qwen3-coder-next', 100)
$hourlyLimits.Add('dashscope-coding-plan/glm-5', 100)
$hourlyLimits.Add('dashscope-coding-plan/glm-4.7', 100)
$hourlyLimits.Add('dashscope-coding-plan/kimi-k2.5', 100)
$hourlyLimits.Add('dashscope-coding-plan/minimax-m2.5', 100)
...
foreach ($model in $modelStats.Keys) {
    ...
    $modelQuota = @{}
    $modelQuota.Add('requests', $usage)
    $modelQuota.Add('hourlyLimit', $hourlyLimit)
    $modelQuota.Add('totalQuota', $totalHourlyQuota)
    $modelQuota.Add('percentage', $percentage)
    $quotaUsage.Add($model, $modelQuota)
}
...
$result.Add('hasApiWarning', $hasApiWarning)
$result.Add('hasMemoryWarning', $hasMemoryWarning)
$result.Add('hasDiskWarning', $hasDiskWarning)
$result.Add('severeMemoryWarning', $severeMemoryWarning)
$result.Add('severeDiskWarning', $severeDiskWarning)
$result.Add('shouldSendNotification', $shouldSendNotification)
$result.Add('notificationType', $notificationType)
$result.Add('isSilentPeriod', $isSilentPeriod)
...
if ($shouldSendNotification) {
    ...
    if ($hasMemoryWarning) {
        $warningValue = if ($hasMemoryWarning) { 90 } else { 85 }
        $alertMessage += "⚠️ **内存使用率高**: $memoryPercent% (> $warningValue%)`n"
    }
    
    if ($hasDiskWarning) {
        $warningValue = if ($hasDiskWarning) { 85 } else { 80 }
        $alertMessage += "⚠️ **磁盘使用率高**: $diskUsage% (> $warningValue%)`n"
    }
    
    if ($severeMemoryWarning) {
        $severeValue = if ($severeMemoryWarning) { 95 } else { 90 }
        $alertMessage += "🚨 **严重内存警告**: $memoryPercent% (> $severeValue%) - 建议重启Gateway`n"
    }
    
    if ($severeDiskWarning) {
        $severeValue = if ($severeDiskWarning) { 95 } else { 90 }
        $alertMessage += "🚨 **严重磁盘警告**: $diskUsage% (> $severeValue%) - 需要清理空间`n"
    }
