...
Write-Host "Updating $($jobIds.Count) jobs to use $newModel..."
Write-Host ""

# 使用 openclaw CLI 直接更新
$count = 0
foreach ($jobId in $jobIds) {
    try {
        $result = openclaw cron update --id $jobId --model $newModel 2>&1
        if ($LASTEXITCODE -eq 0) {
            $count++
            Write-Host "[$count] Updated: $jobId"
        } else {
            Write-Host "[!] Failed: $jobId - $result"
        }
    } catch {
        Write-Host "[!] Error: $jobId - $_"
    }
}

Write-Host ""
Write-Host "Total updated: $count / $($jobIds.Count)"
