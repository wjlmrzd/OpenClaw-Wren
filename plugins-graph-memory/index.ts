...
const graphMemoryPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // registerContextEngine only exists in newer openclaw versions (2026.3+)
    if (typeof (api as any).registerContextEngine === "function") {
      (api as any).registerContextEngine("graph-memory", () => engine);
    }
