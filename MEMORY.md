...
13. **Cron announce channel** → `announce` 模式默认 whatsapp（未配置），导致任务反复失败 → **必须显式指定 `channel: telegram`** ✅ 已重建 6 个 job 并禁用旧 job；旧 job 使用完整 UUID 才可禁用
...
> ✅ 2026-04-10 修复：6 个 job 添加 top-level `delivery` 修复 whatsapp 报错
> ✅ 2026-04-11 凌晨修复：6 个 whatsapp job 重建（新建 job + 禁用旧 job）；成本追踪员 timeout 60s→120s；system-mode-state.json 清理无效 UUID
...
| f46cfd66 | Obsidian Config Snapshot | `5 4 * * *` | 凌晨配置快照 |
| 13f18a92 | 🧬 知识演化员 | `30 4 * * *` | 凌晨知识整理 |
| a3a50177 | 🛠️ 每日维护员 | `35 5 * * *` | 凌晨维护 |
| 16c5208a | ⚙️ 配置优化员 | `38 4 * * *` | 凌晨配置优化 |
| 02a7b20b | 🧹 日志清理员 | `42 4 * * *` | 凌晨日志清理 |
| b65e9a07 | 🚨 灾难恢复官 | `15 6 * * 0` | 周日 06:15 |
| f84bb934 | 🏃 跑步-周日 | `18 6 * * 0` | 周日 06:18 |
| 4a3166b3 | 🏃 运动提醒员 | `0 7 * * *` | 每天 07:00 |
...
| 0b2d9131 | 🌐 网站监控员 | `12 8 * * *` | 每天 08:12 |
| b8665efb | 📰 每日信息汇总 | `15 9 * * *` | 每天 09:10 |
...
| e4ce2030 | 💰 成本追踪员 | `0 */6 * * *` | 每6小时，timeout 120s |
...
| 7edd8ef6 | 🔍 Brave Search 配额追踪 | `3 */4 * * *` | 每4小时，>80%或超限发 Telegram 提醒 |
