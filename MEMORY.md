...
## 2026-03-30: Edge �ղؼ��Զ�ͬ��

**�¼�**: ���� Edge ��ǩ�Զ�ͬ��ϵͳ

### ����
- �ű�: scripts/edge-bookmark-sync.ps1
- ��ǩ�ļ�: Edge Ĭ��λ��
- Obsidian: E:\software\Obsidian\vault\knowledge\�ղؼ�ͬ��.md

### Cron ����
- ?? �ղؼ�ͬ�� (697b1445...) - ÿ�� 09:00
...
## 2026-03-31: minimax-2.7 ռ���Ż�

**�¼�**: �����Ż����� minimax-2.7 �� Cron �����е�ռ�������� ~40%

### �������

| ���� | ԭģ�� | ��ģ�� |
|------|--------|--------|
| ?? �ʼ����Ա | qwen3.5-plus | Ĭ�� (minimax-2.7) |
| ?? �糿ժҪ | qwen3.5-plus | Ĭ�� (minimax-2.7) |
| ?? ֪ͨЭ��Ա | qwen3.5-plus | Ĭ�� (minimax-2.7) |
| ?? �����°����� | glm-5 | Ĭ�� (minimax-2.7) |
...
## 2026-03-31: 健康监控员 false宕机 alert 修复

**根因**: openclaw-cn CLI 命令在 PowerShell 非 TTY 模式下挂起。Windows 防火墙或策略阻断了所有到 127.0.0.1:18789 的出站 TCP 连接，导致健康检查总是超时失败，触发 false 宕机告警。

**修复**: 重写 gateway-monitor.ps1 v5，移除所有需要 TCP 连接的检查，改用本地非阻塞检查：
- netstat -ano 端口监听检查
- Get-CimInstance Win32_Process 进程+内存检查
- 配置文件 JSON 有效性

**关键教训**: 在 Windows 上，PowerShell 脚本连接到 127.0.0.1 的 TCP probe 可能会被防火墙/策略拦截，导致永久挂起。使用 netstat 本地查询更可靠。

## 2026-03-31: Cron ���񱨸�ͳһ����ִ�м���ͻỰ����

**�¼�**: Wren Ҫ���������񱨸渽�����ϴ�ִ�м���ͻỰ����

### �޸�����

������ 28 �� cron ����� payload ͳһ����ͷ����ʽ��
...
�����: �������Ա���¼�Э��Ա���ʼ����Ա�������Ż�Ա����������Ա
ά����: ���ݹ���Ա��ÿ��ά��Ա����־����Ա�������Ż�Ա��ϵͳ�Լ�Ա
������: �˶�����Ա����վ���Ա��ÿ���ܽᡢ�ɱ�����ʦ����Ӫ�ܼࡢ�ع����Ա���糿ժҪ
�˶���: �ܲ�ѵ�����ѣ��ܶ�/����/����/���գ���ÿ��ѵ���ع�
ר����: ��Ŀ���ʡ������°����ѡ�ÿ���籨����ȫ���Ա��֪ͨЭ��Ա��RSS���ļ�ء��ղؼ�ͬ����ÿ����Ϣ���ܡ�֪ʶ������Ԫ�顢���ѻָ��١���������Ա

### ��ѵ

- Cron ����ÿ��ִ��ʱ�����ϴ�ִ�м���Ƕ�̬��
- ��ǰ payload �е��ϴ�ִ��ʱ���Ǿ�̬�ģ����𽥹�ʱ
- ���õķ������� Cron ִ��ǰ�� Gateway �Զ�ע�� lastRunAtMs �� nextRunAtMs �� payload
- OpenClaw ��ǰ�汾��֧�� payload ģ������������ֶ�ά������ֵ


## 2026-03-31: 笔记写入规范固化 ✅

**事件**: Wren 明确要求，将"写入笔记→先入 Inbox"规范写入脚本

### 整理规范（obsidian-knowledge-organizer.ps1 头部注释）

```
1. 触发写入规则
   - Wren 要求写入笔记 → 统一先进入 00-Inbox/
...
**核心原则**: 任何人（包括我）写入知识库的内容，都必须经过 Inbox 中转，禁止直达正式目录。

---

## 2026-03-31: 工程知识篇章 - glm-5 专用管理

**事件**: 为 Wren 在 Obsidian 中创建独立的工程知识篇章，使用 glm-5 模型管理
...
| 篇章 | 管理模型 | 用途 |
|------|---------|------|
| 工程知识 | glm-5 | 工程复盘、技术分析、CAD/脚本/架构 |
| 主知识库 | qwen3.5-plus | 通用知识节点、概念整理 |

### 知识互通

- 工程知识篇章使用 [[工程知识/文件名]] 与主知识库互通
- 主会话识别 CAD/自动化/架构等关键词时优先检索工程知识篇章
- 重要工程知识同步到 MEMORY.md

### 配置文档

- scripts/engineering-knowledge-gm5.md - glm-5 管理策略

---

## 2026-03-31: 原子文档笔记系统上线

**事件**: 为 Wren 建立原子文档笔记管理系统，实现文档解析和自动关联
...
1. **文档解析**: PDF/图片/文本 → 原子笔记
2. **来源标注**: 文档编号、版本、条款、适用范围
3. **自动关联**: 纵向（同文档）+ 横向（跨文档）+ 术语关联
4. **组合笔记**: 按需组合不同原子笔记
...
| 脚本 | 功能 |
|------|------|
| atomic-note-template.md | 原子笔记模板 |
| atomic-doc-processor.ps1 | 文档解析器 |
| atomic-linker.ps1 | 自动关联器 |
